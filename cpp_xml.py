...
def make_struct(tag, debug_str=False):
  ...
  for item in tag.getElementsByTagName("field"):
    typeid = type_convert[item.getAttribute("type")]
    identifier = item.getAttribute("name")
    str += "\t" + typeid + " " + identifier + "_;\n"
    str += "\t" + typeid + " " + identifier + \
        "() const {return " + identifier + "_;}; \n"
    str += "\ttemplate<typename T>\n"
    str += "\t" + "void" + " set_" + identifier + \
        "(T&& t){"+identifier + "_ = t;}; \n"
  str += "};\n\n"
  return str
...
class ParseContent:
  ...
  def parse_condation(self, tag, level=1):
    ...
    if tag.nodeName == "if":
      str += head + "if ("
      str += re_prefix_cpp_field(tag.getAttribute("test"),
                                 self.field_prefix, self.feild_type, True)
      str += ") {\n"
      ...
      field_at_for = re_prefix_cpp_field(
          collection, self.field_prefix, self.feild_type, True)
      field_in_for = re_prefix_cpp_field(
          collection, self.field_prefix, self.feild_type, False)
      open = tag.getAttribute("open")
      close = tag.getAttribute("close")
      separator = tag.getAttribute("separator")

      str += print_ss(open, head)
      str += head + "for (size_t i = 0; i < " + \
          field_at_for + ".size(); i++) {\n"
      str += head + \
          "\tif (i != 0) {" + print_ss(separator,
                                       "\n\t\t" + head) + head + "\t}\n"
...
class ParseXml:
  ...
  def cpp_header(self):
    ...
    self.include_header_string += cpp_header_include(
        "sql_handler.h", "\"", "\"")
  ...
  def make_get_sql_func(self, item, param_name):
    ...
    sql_func_content = "static std::string " + \
        self.get_sql_func_name(item.getAttribute("id"))
    if (param_name):
      sql_func_content += " (CommonSqlHandler *sql_handler, const " + \
          param_name + " &param)"
    else:
      sql_func_content += "()"
    sql_func_content += " {\n"
  ...
  def make_get_result_func(self, item, result_name, param_name, get_ret_way):
    ...
    # 获取sql
    self.make_result_func += head + "auto sql = " + \
        self.get_sql_func_name(item.getAttribute("id")) + \
        "(sql_handler, param);\n"
    # 执行sql
    if result_name == "":
    ...
    for i in range(0, len(fields)):
      if get_ret_way == "by_label":
        ...
        self.make_result_func += "\t\tret.back().set_%s(sql_handler->Get%s(\"%s\"));\n" % (
            fields[i].getAttribute(
                "name"), type_convert_sql_handler[fields[i].getAttribute("type")],
            fields[i].getAttribute("name"))
      elif get_ret_way == "by_index":
        self.make_result_func += "\t\tret.back().set_%s(sql_handler->Get%s(%d));\n" % (
...
def main():
  for arg in sys.argv[1:]:
    ...
    parse = ParseXml(arg)
    parse.draw(os.path.splitext(arg)[0] + "_sql.cpp")
    print("finish : " + arg)
